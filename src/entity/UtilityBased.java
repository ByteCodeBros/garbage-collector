...
public class UtilityBased extends Agent {
    Map state;

    boolean collected3 = false;

    public UtilityBased(int xPosition, int yPosition, Map map) {
    ...
    private ArrayList<ArrayList<Integer>> findPath(Map map, int xPos, int yPos){
        return PathFinder.findShortestPath(map.getMatrix(), xPos, yPos, this, this.collected3);
    }
    ...
    public void update(Map map, int opXPos, int opYPos){
        if(this.state.isEmpty()){
            ...
            if(Objects.equals(posValue, "1") || Objects.equals(posValue, "3")){ // se tiver lixo onde esta - come lixo
                ...
                System.out.println("UtilityB: pos("+getxPosition()+","+getyPosition() + ") /score:" +getScore());
            } else {
...
//        System.out.println("----------------------");
//        System.out.println("States(UtilityBased): ");
...
//        System.out.println("----------------------");
    ...
    protected String lookUp(Map map, int opXPos, int opYPos){
        ...
        if(garbageList.isEmpty() && !(this.collected3)){
